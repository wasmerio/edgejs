...
void QueueDestroy(SignalWrap* wrap) {
  ...
  // UnodeAsyncWrapQueueDestroy(wrap->env, static_cast<double>(wrap->async_id));
}
...
void OnSignal(uv_signal_t* handle, int signum) {
  ...
  UnodeMakeCallback(wrap->env, self, onsignal, 1, argv, &ignored);
}
...
napi_value SignalCtor(napi_env env, napi_callback_info info) {
  ...
  wrap->env = env;
  // wrap->async_id = UnodeAsyncWrapNewAsyncId(env);
...
void UnodeInstallSignalWrapBinding(napi_env env) {
  ...
  napi_set_named_property(env, global, "__unode_signal_wrap", binding);
}
