...

{
  ...
  // Defer loading source_map_cache until it is actually needed.
  // Source maps are disabled by default, so most processes never load this
  // module at all. The module's initial state already has enabled:false,
  // so lazy access is safe without any eager initialization.
  let sourceMapsCache;
  function getSourceMapsCache() {
    if (sourceMapsCache === undefined) {
      sourceMapsCache = require('internal/source_map/source_map_cache');
    }
    return sourceMapsCache;
  }
  ...
  ObjectDefineProperty(process, 'sourceMapsEnabled', {
    ...
    configurable: true,
    get() {
      return getSourceMapsCache().getSourceMapsSupport().enabled;
    },
  });
  process.setSourceMapsEnabled = function setSourceMapsEnabled(val) {
    getSourceMapsCache().setSourceMapsSupport(val, {
      __proto__: null,
  ...
  // The C++ land calls back when code is generated by eval() or new Function()
  // with a sourceMappingURL comment. The cache module is loaded lazily on the
  // first actual call so that the cost is not paid at bootstrap.
  setMaybeCacheGeneratedSourceMap(function lazyMaybeCacheGeneratedSourceMap(content) {
    return getSourceMapsCache().maybeCacheGeneratedSourceMap(content);
  });
}
