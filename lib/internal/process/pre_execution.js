...
function initializeSourceMapsHandlers() {
  ...
  // When source maps are disabled and code coverage is not active, skip the
  // module load entirely. The cache module's default state is already
  // enabled:false, so no explicit initialization is needed.
  if (!enabled && !process.env.NODE_V8_COVERAGE) {
    return;
  }
  const {
