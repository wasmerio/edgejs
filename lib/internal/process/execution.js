...
function evalScript(name, body, breakFirstLine, print, shouldLoadESM = false) {
  // Only read globalThis.module if it has been explicitly set as a data
  // property (e.g. by the REPL). Accessing a lazy getter here would load the
  // entire 'module' builtin and its transitive deps on every eval startup.
  const moduleDescriptor = ObjectGetOwnPropertyDescriptor(globalThis, 'module');
  const origModule = moduleDescriptor && !moduleDescriptor.get ? moduleDescriptor.value : undefined;
