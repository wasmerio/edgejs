...
class Worker extends EventEmitter {
  constructor(filename, options = kEmptyObject) {
    ...
    // Start after the constructor returns. Besides matching Worker's
    // asynchronous startup contract, this lets an immediate postMessage()
    // publish transferable nested WebAssembly objects before the WASIX child
    // is assigned to a browser worker.
    process.nextTick(() => {
      if (this[kHandle] !== null)
        this[kHandle].startThread();
      process.emit('worker', this);
    });
