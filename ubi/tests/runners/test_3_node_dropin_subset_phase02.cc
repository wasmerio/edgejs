...
#define DEFINE_RAW_NODE_TEST(test_name, script_name)            \
  TEST_F(Test3NodeDropinSubsetPhase02, test_name) {             \
    if (std::getenv("UBI_SKIP_DGRAM_TESTS") != nullptr &&     \
...
// Raw Node tty tests (drop-in from node/test/{parallel,pseudo-tty})
...
// Raw Node http tests (drop-in from node/test/parallel, excluding async-hooks-specific cases)
