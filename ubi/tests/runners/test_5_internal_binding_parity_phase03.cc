...
const credEnvVar = 'UBI_INTERNAL_BINDING_CREDENTIALS_SAFE_GETENV';
process.env[credEnvVar] = 'present';
assert.strictEqual(credentialsBinding.safeGetenv(credEnvVar), 'present');
assert.strictEqual(credentialsBinding.safeGetenv('UBI_INTERNAL_BINDING_MISSING_ENV'), undefined);
