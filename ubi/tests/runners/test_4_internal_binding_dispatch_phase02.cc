...
globalThis.__ubi_internal_binding_dispatch_count = bindingNames.length;
)JS";
...
TEST_F(Test4InternalBindingDispatchPhase02, AllResolversAreReachableAndCached) {
  ...
  std::string error;
  const int exit_code = UbiRunScriptSource(s.env, kDispatchCoverageScript, &error);
  ...
  ASSERT_EQ(napi_get_named_property(
                s.env, global, "__ubi_internal_binding_dispatch_count", &count_value),
            napi_ok);
