...
process._kill = function ubiKill(pid, signal) {
...
function buildEnvPairs(envObj, cwd) {
  ...
  // Node's test/common global leak checks are not compatible with ubi's
...
function resolveForkExecPath(candidate) {
  ...
  if (!base.startsWith('ubi_test_')) return execPath;
  const sibling = path.resolve(path.dirname(execPath), '..', 'ubi');
  try {
...
function _forkChild(fd, serializationMode) {
  if (process.__ubiIpcInitialized) return process.channel;
  const p = new Pipe(PipeConstants.IPC);
  const openRc = p.open(fd);
  if (openRc !== 0) {
    process.__ubiIpcBootstrapError = `Pipe.open(${fd}) failed: ${openRc}`;
    return undefined;
  }
  p.unref();
  const control = setupChannel(process, p, serializationMode || 'json');
  process.__ubiIpcInitialized = true;
...
function spawn(file, args, options) {
  ...
  if (child.stdin) {
    ...
    child.__ubiStdinTotalBytes = () => totalInputBytes;
  }
  ...
  process.nextTick(() => {
    const totalInputBytes = child.stdin && typeof child.__ubiStdinTotalBytes === 'function' ?
      child.__ubiStdinTotalBytes() : 0;
