...
class JsUdpWrap final : public UbiUdpWrapBase {
 public:
  explicit JsUdpWrap(napi_env env_in) : env(env_in), async_id(UbiAsyncWrapNextId(env_in)) {}
  ...
  ssize_t Send(uv_buf_t* bufs, size_t nbufs, const sockaddr* addr) override {
    ...
    UbiUdpSendWrap* send_wrap = listener()->CreateSendWrap(total_len);
    ...
    if (UbiMakeCallback(env, self, onwrite, 3, argv, &result) != napi_ok || result == nullptr) {
  ...
  int32_t provider_type = kUbiProviderJsUdpWrap;
  int64_t async_id = -1;
};
...
void JsUdpWrapFinalize(napi_env env, void* data, void* /*hint*/) {
  ...
  if (wrap->async_id > 0) UbiAsyncWrapQueueDestroyId(env, wrap->async_id);
...
napi_value JsUdpWrapOnSendDone(napi_env env, napi_callback_info info) {
  ...
  UbiUdpSendWrap* req_wrap = UbiUdpWrapUnwrapSendWrap(env, argv[0]);
...
napi_value JsUdpWrapAsyncReset(napi_env env, napi_callback_info info) {
  ...
  napi_get_cb_info(env, info, &argc, nullptr, &self, nullptr);
  JsUdpWrap* wrap = UnwrapJsUdpWrap(env, self);
  if (wrap != nullptr) UbiAsyncWrapReset(env, &wrap->async_id);
  napi_value undefined = nullptr;
  napi_get_undefined(env, &undefined);
  return undefined;
}
...
napi_value JsUdpWrapGetProviderType(napi_env env, napi_callback_info info) {
  ...
  napi_get_cb_info(env, info, &argc, nullptr, &self, nullptr);
  JsUdpWrap* wrap = UnwrapJsUdpWrap(env, self);
  return MakeInt32(env, wrap != nullptr ? wrap->provider_type : kUbiProviderNone);
}

}  // namespace

napi_value UbiInstallJsUdpWrapBinding(napi_env env) {
