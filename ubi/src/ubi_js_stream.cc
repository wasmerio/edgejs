...
void JsStreamDestroy(UbiStreamBase* base) {
  delete FromBase(base);
}

const UbiStreamBaseOps kJsStreamOps = {
    nullptr,
    nullptr,
    nullptr,
    JsStreamDestroy,
...
napi_value CallOnWrite(napi_env env,
  ...
  if (array == nullptr) {
    ...
    return UbiStreamBaseMakeInt32(env, UV_EPROTO);
  }
...
napi_value JsStreamCtor(napi_env env, napi_callback_info info) {
  ...
  wrap->env = env;
  UbiStreamBaseInit(&wrap->base, env, &kJsStreamOps, kUbiProviderJsStream);
...
napi_value JsStreamWritev(napi_env env, napi_callback_info info) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    bool refable = false;
    std::string temp_utf8;
    UbiStreamBaseExtractByteSpan(env, chunk, &data, &len, &refable, &temp_utf8);
    chunks.emplace_back(data, data + len);
    total += len;
  }
...
napi_value JsStreamReadBuffer(napi_env env, napi_callback_info info) {
  ...
  while (len != 0) {
    ...
    if (!UbiStreamEmitRead(&wrap->base.listener_state, static_cast<ssize_t>(available), &buf) && buf.base != nullptr) {
      free(buf.base);
    }
  }
  return UbiStreamBaseUndefined(env);
