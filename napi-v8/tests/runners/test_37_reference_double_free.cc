...
class Test37ReferenceDoubleFree : public FixtureTestBase {};

TEST_F(Test37ReferenceDoubleFree, PortedCoreFlow) {
  ...
  ASSERT_EQ(napi_set_named_property(s.env, global, "__trdf", exports), napi_ok);
...
{ new __trdf.MyObject(true); }
{ new __trdf.MyObject(false); }
__trdf.deleteImmediately({});
)JS"));
}
