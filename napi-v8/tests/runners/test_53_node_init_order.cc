...
class Test53NodeInitOrder : public FixtureTestBase {};

TEST_F(Test53NodeInitOrder, PortedCoreFlow) {
  ...
  ASSERT_EQ(napi_set_named_property(s.env, global, "__tio", exports), napi_ok);
...
if (__tio.cppIntValue !== 42) throw new Error('cppIntValue');
if (__tio.cppStringValue !== '123') throw new Error('cppStringValue');
)JS"));
}
